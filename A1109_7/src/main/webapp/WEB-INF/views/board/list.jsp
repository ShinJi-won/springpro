...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$(".paginate_button a").on("click",function(e){
			//현재 a태그를 클릭하면 이벤트가 e로 넘어옴
			e.preventDefault(); //a tag기능을 막는부분 ->서버에 전송되면 안된다
			//세개 중에 하나a태그 클릭하면 이동막아야함
			var page=$(this).attr("href");
			//페이지 번호 가져온다
			pageFrm.find("#page").val(page);
			//form에 id가 page인것을찾아 page값넣어준다
			pageFrm.submit();
...
<body>
	...
	<c:if test="${empty mvo }">
		...
		<form class="form-inline" action="${root }/login/loginProcess"
			method="post">
	...
	<c:if test="${!empty mvo }">
		<form class="form-inline" action="${root }/login/logoutProcess"
			method="post">
			<div class="form-group">
	...
	<div class="panel-body">
		<table class="table table-bordered table-hover">
					...
					<td><c:if test="${vo.boardLevel>0 }">
						...
						<c:if test="${vo.boardLevel>0 }">
							<c:if test="${vo.boardAvailable==1 }">
								<a class="move" href="${vo.idx}"><c:out
										value='[RE]${vo.title }' /></a>
							</c:if>
			...
			<c:if test="${!empty mvo }">
				...
				<tr>
					<td colspan="5">
						<button id="regBtn" class="btn btn-md pull-right">글쓰기</button>
					</td>
				</tr>
			</c:if>
		</table>
  ...
  <div class="form-group">
    <input type="text" value="${pageMaker.cri.keyword }" name="keyword" class="form-control" id="email">
  </div>
